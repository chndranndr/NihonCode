...
| `npm run check`           | typecheck + lint + format:check + arch + taste + docs links + audit self-test + data audit                                                                                                |
| `npm test`                | Vitest unit suite (jsdom)                                                                                                                                                                 |
| `npm run eval`            | Playwright smoke eval: builds, serves, drives desktop + mobile viewports                                                                                                                  |
| `npm run doctor`          | Full health report (manifest, CI YAML, check, test, gc dry-run; `--with-eval` adds browser eval); `--json` for machines                                                                   |
...
| `npm run audit:data`      | Raw-data baseline comparison + clean-slice gate; `--self-test` proves gate failure paths; `--json` for the full report; `--write-baseline` only for a deliberate, reviewed dataset change |
...
All under `scripts/`, all dependency-light (Node stdlib + `js-yaml` in doctor):
...
| `check-docs.mjs`     | Relative links + anchors in AGENTS.md and docs/                                          |
| `audit-data.mjs`     | Raw `data/` structural drift + MVP clean-slice gate ([data-quality.md](data-quality.md)) |
