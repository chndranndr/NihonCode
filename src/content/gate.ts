 ...
 * gate"). The app's only data source is data/clean/ — generated by
 * scripts/build-clean.mjs from the frozen raw scrape plus owner
 * adjudications in curation/adjudications.json. These validators are the last
 * line of defense between that generated pool and the typed domain models:
 * anything malformed is flagged and excluded from the graded pool, never
 * thrown on.
 *
 ...
 * non-empty, and only graded grammar lessons enter the pool, with
 * answer-in-choices quizzes.
 */

import { z } from "zod";

import { grammarLessonId, kanaId, kanjiId, vocabId, type JlptLevel } from "./ids";
import type { Flag, GateResult, GrammarLesson, KanaItem, KanjiItem, VocabItem } from "./models";
