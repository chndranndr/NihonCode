...
2. **Conjugation drill** (PRD §10.9). Pure domain conjugator, unit-tested over every class and form including exceptions; drill builder; user chooses forms and question count; eligible-word preview before start; grading; replace the CONJUGATION `LockedPanel` uses with the live drill; e2e conjugation flow. Acceptance: every PRD §10.9 bullet; sampled godan/ichidan/irregular and i-/na-adjective grading correct (implementation_plan Phase 3 DoD).
