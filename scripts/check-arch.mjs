...
//   1. data-gate: only src/content/ may import from data/generated or data/jlpt.
...
for (const file of files) {
  ...
  for (const { spec, line } of extractSpecifiers(source)) {
    ...
    if (target.kind === "raw-data" && !(ownLayer === "content")) {
      add(file, line, "data-gate", `imports raw dataset "${spec}"; only src/content/ may`);
    }
